...
function initCharts() {
  ...
  const ccols   = categories.map(c => c.color);
...
function renderProds() {
  ...
  : `<div class="ie" style="background:${getCatColor(p.category)}22">${getCatIcon(p.category)}</div>`;
...
function openProd(id = null) {
  ...
  const catInput = $('pCa');
  const catLbl   = $('catPickerLabel');
  const catVal   = p?.category || (categories[0]?.name || '');
  if (catInput) catInput.value = catVal;
  if (catLbl)   { catLbl.textContent = catVal || 'Select a category…'; catLbl.style.color = catVal ? 'var(--accent)' : ''; }
  catPickerOpen = false;
  $('catPickerDropdown')?.classList.remove('open');
  $('catPickerHeader')?.classList.remove('open');
  renderCatPicker();
  $('pQ').value  = p?.quantity  ?? '';
  $('pPr').value = p?.price     ?? '';
  $('pRe').value = p?.reorder   ?? '';
...
function loadCategories() {
  fetch('/categories')
    ...
    .then(data => {
      ...
      loadProducts(); // load products after categories are ready
    });
}
...
function getCatColor(name) {
  return categories.find(c => c.name === name)?.color || '#4db8ff';
}
...
function renderCatPicker() {
  ...
  if (!categories.length) {
    list.innerHTML = '<span class="cat-empty">No categories yet. Add one below.</span>';
    return;
  }
  const selected = document.getElementById('pCa')?.value || '';
  list.innerHTML = categories.map(c =>
    `<span class="cat-tag${selected === c.name ? ' selected' : ''}" onclick="selectCat('${esc(c.name)}')">
      ${esc(c.name)}
      <span class="cat-del" title="Delete category" onclick="deleteCategoryInline(event,'${esc(c.name)}',${c.id})">✕</span>
    </span>`
  ).join('');
  const lbl = document.getElementById('catPickerLabel');
  if (lbl) lbl.textContent = selected || 'Select a category…';
}

function toggleCatPicker() {
  catPickerOpen = !catPickerOpen;
  const dd  = document.getElementById('catPickerDropdown');
  const hdr = document.getElementById('catPickerHeader');
  dd?.classList.toggle('open', catPickerOpen);
  hdr?.classList.toggle('open', catPickerOpen);
  if (catPickerOpen) renderCatPicker();
}

function selectCat(name) {
  const input = document.getElementById('pCa');
  if (input) input.value = name;
  const lbl = document.getElementById('catPickerLabel');
  if (lbl) { lbl.textContent = name; lbl.style.color = 'var(--accent)'; }
  renderCatPicker();
  // close after selection
  catPickerOpen = false;
  document.getElementById('catPickerDropdown')?.classList.remove('open');
  document.getElementById('catPickerHeader')?.classList.remove('open');
}

/* Auto-assign a color from a palette based on how many categories exist */
const CAT_PALETTE = ['#4db8ff','#3dffa0','#ff6b35','#ffcc00','#ff4d6d','#e8ff47','#b388ff','#69f0ae','#ff8a65','#40c4ff','#f48fb1','#a5d6a7'];
...
function addCategoryInline() {
  ...
  if (categories.find(c => c.name.toLowerCase() === name.toLowerCase())) {
    toast('Category already exists', 'err'); inp.value = ''; return;
  }

  /* Pick next color + icon automatically */
  const idx   = categories.length % CAT_PALETTE.length;
  const color = CAT_PALETTE[idx];
  const icon  = CAT_ICONS[idx];
  ...
  .then(data => {
    if (data.success && data.id) {
      categories.push({ id: data.id, name, color, icon });
      inp.value = '';
      selectCat(name);
      toast('Category added', 'ok');
      logActivity('var(--accent)', `Category <strong>${esc(name)}</strong> added`);
    } else {
      const msg = data.errors?.name?.[0] || data.message || 'Failed to add category';
      toast(msg, 'err');
    }
  })
  .catch(() => toast('Failed to add category', 'err'));
}

function deleteCategoryInline(e, name, id) {
  e.stopPropagation();
  ...
  .then(() => {
    ...
    const current = document.getElementById('pCa')?.value;
    if (current === name) {
      const input = document.getElementById('pCa');
      if (input) input.value = '';
      const lbl = document.getElementById('catPickerLabel');
      if (lbl) { lbl.textContent = 'Select a category…'; lbl.style.color = ''; }
    }
    renderCatPicker();
    toast('Category deleted', 'ok');
    logActivity('var(--danger)', `Category <strong>${esc(name)}</strong> deleted`);
  })
  .catch(() => toast('Failed to delete category', 'err'));
}

// Close picker when clicking outside
document.addEventListener('click', e => {
  if (catPickerOpen && !e.target.closest('.cat-picker')) {
    catPickerOpen = false;
    document.getElementById('catPickerDropdown')?.classList.remove('open');
    document.getElementById('catPickerHeader')?.classList.remove('open');
  }
